...
type
    ...
    // список расписани€, например комманд. ѕредполагает возможность указани€ даты и врем€
...
begin
 ...
 for i := ReadIndex to Count - 1 do
  begin
   s := Trim (Strings[i]);
   if Pos ('[', s) = 1 then break; // найдена следующа€ секци€
   p := Pos(' = ', s);
   // оптимизаци€ некоторых ini-строк с пробелами
   if p > 0 then
      begin
       System.Delete(s, p, 3);
       System.Insert('=', s, p);
 ...
 for n := 0 to Count - 1 do
  begin
   ...
   if Pos ('[' + sect + ']', s) = 1 then
      save := TRUE; // со след. строки добавл€ть
 end;
...
begin
 ...
 if Count > 0 then
   for n := 0 to Count - 1 do
    self [n] := Items [n].FileName; // ¬ернуть имена вместо кода сортировки
