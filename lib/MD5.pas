...
type
  ...
  TMD5Vector = packed array[0..15] of Byte;
  ...
  PMD5Vector = ^TMD5Vector;
  PMD5Digest = ^TMD5Digest;

  // вычисление хеш-суммы для строки
function MD5StringA(const S: AnsiString): TMD5Digest;
function MD5StringW(const S: WideString): TMD5Digest;
...
function MD5StringW(const S: WideString): TMD5Digest;
begin
  Result := MD5Buffer( PWideChar(S)^, Length(S) * 2 );
end;
